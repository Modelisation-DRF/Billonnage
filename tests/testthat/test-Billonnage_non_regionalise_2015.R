test_that("Test de la fonction ABCD_DHP215() avec MSCR", {
  ...
  Billonage <- Billonage %>% mutate(bilonID = seq_len(nrow(Billonage)))

  fic_intrant <- Billonage %>% dplyr::select(Placette, ArbreID, Espece, DHPcm, MSCR, bilonID)

  result_attendu <- Billonage %>% filter(!is.na(MSCR)) %>% mutate(DER=NA, type="MSCR_P") %>% dplyr::select(bilonID, type, F2, F3, F4, P, F1, DER)
...
test_that("Test de la fonction ABCD_DHP215() avec 1234", {
  ...
  expect_no_error(ABCD_DHP215(data=fic_intrant, type="1234"))
